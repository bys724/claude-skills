...
Zotero에서 area/image 어노테이션으로 캡처한 그림·표·수식은 노트에 `![[Sources/paper imgs/[노트명]/img-*.png]]` 임베드로만 들어오고 **설명 텍스트가 없다**. 캡션이 없으면 나중에 노트를 훑을 때 무슨 그림인지·왜 캡처했는지 알 수 없다. 자주 누락되는 기본작업이므로 **모든 캡처 이미지**에 한글 설명을 단다.

**대상 (두 군데 모두):**
- 🖼️ Key Figures — 사용자가 핵심으로 고른 그림
- 본문 하이라이트/어노테이션 영역에 흩어진 캡처 이미지 — 읽으면서 area 캡처한 것

**작업 방식:** 각 이미지 파일을 `Read`로 **실제로 열어 내용을 확인한 뒤** 캡션 작성. 파일명·주변 텍스트만 보고 추정하지 않는다 — 이미지를 직접 봐야 정확한 설명이 나온다.

```markdown
**Figure N: [간략한 제목]**
![[Sources/paper imgs/[노트명]/img-N.png]]
*[한글 설명: 무엇을 보여주는지 + 가능하면 왜 캡처했는지·논문에서의 역할]*
```

- 캡처 이미지가 많으면 전부에 1-2문장. 핵심 그림은 좀 더 자세히
- PDF에 영어 원본 캡션이 있으면 참고하되, 빠르게 이해되는 한글로 다시 씀
- 캡션은 임베드 **아래 본문**에만 추가. 이미지 파일·어노테이션 데이터 자체는 건드리지 않음 (맨 아래 "하지 말 것" 참조)
...
- 이미지 파일·어노테이션 데이터 수정 (플러그인이 자동 임포트) — 단, 임베드 아래 한글 캡션 추가는 6번에서 수행하는 정상 작업이므로 스킵 금지
